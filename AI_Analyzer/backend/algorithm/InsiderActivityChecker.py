...
class InsiderActivityChecker:
    ...
    # We only care about P and S â€” genuine open-market intent.
    ...
    def _fetch_one(self, ticker: str) -> tuple[str, list[dict]]:
        ...
        try:
            ...
            for tx in raw:
                ...
                # â”€â”€ CRITICAL: Filter out 10% Beneficial Owners â”€â”€
                ...
                # or secondary offerings â€” NOT genuine conviction trades.
                ...
                if value > self._MAX_SANE_VALUE:
                    print(f"  [SEC] Filtered suspiciously large tx: {filer_name} "
                          f"${value:,.0f} ({ticker}) â€” likely institutional")
                    continue

                # Extract title â€” Finnhub puts role info in the name field
                insider_title = ""
            ...
            # Sort by value descending â€” most significant trades first
            ...
            if transactions:
                ...
                print(f"  [SEC] {ticker}: {len(transactions)} insider tx (14d) â€” "
...
# LIVE MARKET STATE â€” Real-time VIX, SPY, pre-market detection, earnings season
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
